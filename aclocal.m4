...
# NTOP_TEST_MUSTHAVE(itemname, headername, functionname, VARNAME)
# ---------------------------------------------------------------
AC_DEFUN([NTOP_TEST_MUSTHAVE],
[dnl
# Expansion of NTOP_NTOP_TEST_MUSTHAVE($1, $2, $3, $4)
...
if eval "test \"\${$nt_ac_lib}\" = yes" &&
   eval "test \"\${$nt_ac_header}\" = yes"; then
    ...
    if eval "test \"\${$nt_ac_header}\" = yes"; then
         ...
         AC_DEFINE_UNQUOTED(HAVE_$4_H, 1, [Show we found $2 and set INCS])
    elif test -d /usr &&
         test -d /usr/include &&
         ...
         AC_DEFINE_UNQUOTED(HAVE_$4_H, 1, [Show we found $2])
    else
        AC_MSG_RESULT(error)
        echo
        echo "******************************************************************"
    ...
    if eval "test \"\${$nt_ac_lib}\" = yes"; then
        ...
        AC_DEFINE_UNQUOTED(HAVE_$4, 1, [Show we found -l$1 and set LIBS])
    elif test -d /usr &&
         test -d /usr/lib &&
    ...
    dnl> Can we find $2 and lib$1 where s/he told us?
         ...
         test -r ${$4_ROOT}/$2; then
         AC_MSG_RESULT([ok, found in \${$4_ROOT}])
         INCS="${INCS} -I\${$4_ROOT}"
         AC_DEFINE_UNQUOTED(HAVE_$4_H, 1, [Show we found $2 and set INCS])
    elif test -d ${$4_ROOT} &&
       test -d ${$4_ROOT}/include &&
       test -r ${$4_ROOT}/include/$2; then
         AC_MSG_RESULT([ok, found in \${$4_ROOT}/include])
         INCS="${INCS} -I${$4_ROOT}/include"
         AC_DEFINE_UNQUOTED(HAVE_$4_H, 1, [Show we found $2 and set INCS])
    elif test -d ${$4_ROOT} &&
       test -d ${$4_ROOT}/include &&
       test -d ${$4_ROOT}/include/$1 &&
       test -r ${$4_ROOT}/include/$1/$2; then
         AC_MSG_RESULT([ok, found in ${$4_ROOT}/include/$1])
         INCS="${INCS} -I${$4_ROOT}/include/$1"
         AC_DEFINE_UNQUOTED(HAVE_$4_H, 1, [Show we found $2 and set INCS])
    else
        AC_MSG_RESULT(error)
        echo
    ...
    if eval "test \"\${$nt_ac_lib}\" = yes"; then
        ...
        AC_MSG_RESULT([ok, library found in ${$4_ROOT}])
        AC_DEFINE_UNQUOTED(HAVE_$4, 1, [Show we found -l$1 and set LIBS])
    fi
fi
