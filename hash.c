...
void freeHostInfo(HostTraffic *host, int actualDeviceId) {
  ...
  if(myGlobals.isLsofPresent) {
	...
	for(j=0; j<MAX_NUM_CONTACTED_PEERS; j++)
	  if(myGlobals.processes[i]->contactedIpPeersSerials[j] == host->hostSerial)
	    myGlobals.processes[i]->contactedIpPeersSerials[j] = FLAG_NO_PEER;
      }
    }
  }
	...
	while(scanner != NULL) {
	  if(scanner->element != NULL) {
	    for(i=0; i<MAX_NUM_CONTACTED_PEERS; i++) {
	      if(scanner->element->contactedIpPeersSerials[i] == host->hostSerial)
		scanner->element->contactedIpPeersSerials[i] = FLAG_NO_PEER;
	    }
	  }
